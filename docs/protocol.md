...
| `POST /api/reveal` | `{ path }` → `{ ok, path }`: reveal the file in Finder (`open -R`; `PINPOINT_OPEN` names another opener). `~` is the server user's home, a relative path is under the project root; `400` without a string path, `404` when nothing is there, `501` off macOS with no opener set |
